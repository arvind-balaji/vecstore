...
test_that("Finding nearest neighbors returns expected results with HNSWIndex", {
  index <- HNSWIndex$new(dim = 2, size = 10)
  vs <- VectorStore$new(index)
...
test_that("Finding nearest neighbors returns expected results with KNNIndex", {
  index <- KNNIndex$new(dim = 2)
  vs <- VectorStore$new(index)
  vectors <- list(c(1, 1), c(2, 2), c(3, 3))
  non_vectors <- list(list(name = "John", age = 30), list(name = "Alice", age = 25), list(name = "Bob", age = 35))
